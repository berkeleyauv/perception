...
class SlalomDetector:
    ...
    def detect_white_pole_target(
        ...
        if method == "contrast":
            ...
            red_pipe = self._choose_paired_red_pipe(red_candidates, white_pipe, width)
    ...
    def _choose_paired_red_pipe(
        ...
        return max(
            ...
            key=lambda pipe: (
                pipe.score
                + 0.20 * min(pipe.height / white_height, white_height / max(float(pipe.height), 1.0))
                - 0.10 * abs(pipe.center_x - image_width / 2) / image_width
            ),
        )
...
def pipe_detection_to_dict(pipe: PipeDetection | None) -> dict[str, Any] | None:
    if pipe is None:
        return None
    return {
        "label": pipe.label,
        ...
        "area": pipe.area,
        "score": pipe.score,
        "aspect_ratio": pipe.aspect_ratio,
        "fill_ratio": pipe.fill_ratio,
        "width_stability": pipe.width_stability,
        "vertical_alignment": pipe.vertical_alignment,
        "contrast_strength": pipe.contrast_strength,
        "center_x": pipe.center_x,
        "center_y": pipe.center_y,
        "bottom_y": pipe.bottom_y,
    }


def slalom_estimate_to_raw(estimate: SlalomEstimate) -> dict[str, Any]:
    return {
        "valid": estimate.valid,
        "red_pipe": pipe_detection_to_dict(estimate.red_pipe),
        "left_white_pipe": pipe_detection_to_dict(estimate.left_white_pipe),
        "right_white_pipe": pipe_detection_to_dict(estimate.right_white_pipe),
    }


@register_perceiver(task="slalom", algo="classical")
class SlalomClassicalPerceiver(TaskPerceiver):
    """TaskPerceiver wrapper around urb_slalom's classical detector."""

    def __init__(self, detector: SlalomDetector | None = None):
        super().__init__()
        self.detector = detector or SlalomDetector()

    def predict(self, frame: np.ndarray, context: TaskContext | None = None) -> PerceptionOutput:
        context = context or TaskContext()
        pass_side_value = context.metadata.get("pass_side", PassSide.LEFT)
        method = context.metadata.get("method", "hsv")
        if not isinstance(pass_side_value, PassSide):
            pass_side_value = PassSide(pass_side_value)

        estimate = self.detector.detect(frame, pass_side=pass_side_value, method=method)
        debug_frames = {}
        if context.debug:
            debug_frames["annotated"] = self.detector.annotate(frame, estimate)
        ...
        return PerceptionOutput(
            result=DetectionResult(
                ...
                yaw_error=estimate.yaw_error,
                confidence=estimate.confidence,
                frame_id=context.frame_id,
                raw=slalom_estimate_to_raw(estimate),
            ),
