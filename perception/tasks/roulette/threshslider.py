...
low_H = 86  # 49#29#0
low_S = 141  # 77#0
low_V = 0  # 36#0   For Small sector, increasing lower V bound reduces
high_H = 106  # 130#max_value_H
high_S = 217  # 250#max_value
high_V = 168  # max_value
...
"""
def findLikelyGate(rectList, contours):
    if not (rectList and contours):
        return
    try:
        closest = min(contours, key=lambda x: abs(cv.minAreaRect(x)[0][1] - cv.minAreaRect(rectList[0])[0][1]) + abs(1/(cv.minAreaRect(x)[0][0] - cv.minAreaRect(rectList[0])[0][0])))
        rectList.append(closest)
    except:
        return
"""
...
def heuristic(contour):
    ...
    cent = rect[0]
    dist = 0
    if len(likelyGate) > 1 and allLarger(60):
