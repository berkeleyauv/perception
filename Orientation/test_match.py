...
L_THRESHOLD   = 60 # max L threshold for black

# In the LAB color space, A > 128 = red, A < 128 = green
# magnitude correlates to saturation
A_THRESHOLD   = 135 # min A threshold for red

MIN_BLOB_AREA = 50 # noise filter
TOLERANCE     = 0.04 # prevents unnecessary adjustments with tiny mis-alignments

COLOR_LEFT    = (50,  220,  50) # BGR Green - color for left post
COLOR_RIGHT   = (50,   50, 220) # BGR Red - color for right post
COLOR_GATE    = (0,   165, 255) # BGR Orange - for "YOLO" Bounding Box
COLOR_DASH_BG = (10,   10,  10) # BGR Black - for background
...
# ══════════════════════════════════════════════════════════════════════════════
# Interactive gate box drawing -> YOLO Stand In
# ══════════════════════════════════════════════════════════════════════════════
...
def segment_black_lab(crop: np.ndarray,
    ...
    # Pixel below l_thresh -> set to 255, else set to 0
...
if __name__ == "__main__":
    ...
    crop, black_mask, red_mask, left_stats, right_stats, divider_y = \
        detect_posts(img, gate_roi, l_thresh, a_thresh)
    ...
    # Colour-coded masks for display
    gx, gy, gw, gh = gate_roi
    mid_x = gw // 2

    black_colour = cv2.cvtColor(black_mask, cv2.COLOR_GRAY2BGR)
    red_colour   = cv2.cvtColor(red_mask,   cv2.COLOR_GRAY2BGR)
